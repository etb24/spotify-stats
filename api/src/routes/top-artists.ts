...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const spotifyRes = await axios.get('https://api.spotify.com/v1/me/top/artists', {
    ...
    if (err instanceof SpotifyTokenError) {
      ...
      console.error('artist_tokens_failed:', err.cause ?? err);
    ...
    if (axios.isAxiosError(err)) {
      ...
      console.error('spotify_top_artists_failed:', status, data || err.message || err);
    ...
    console.error('top_artists_unknown_error:', err);
